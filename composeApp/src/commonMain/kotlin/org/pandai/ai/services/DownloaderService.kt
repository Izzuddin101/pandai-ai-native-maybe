...
typealias DownloadResult = Pair<String, Float>
    ...
    fun download(
        ...
        runCatching {
            httpClient.prepareGet(downloadUrl) {
                ...
                handleExecutor(it, destPath, fileSystem, downloadUrl)
            }
        }
    }

    private suspend fun FlowCollector<Result<DownloadResult, String>>.handleExecutor(
        ...
        if (response.status.isSuccess()) {
            ...
            val errorMessage =
                "Failed to download file ${downloadUrl}. HTTP Status: ${response.status.value}. ${response.bodyAsText()}"
            Logger.e(errorMessage)
            emit(Err(errorMessage))
        }
    }
}